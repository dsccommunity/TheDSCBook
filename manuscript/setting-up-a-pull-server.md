...
At this point, you should have almost all the basics that you need to begin using DSC. At this point in the book, I'm going to start shifting from simple examples over to more production-scale ones, and that means we're going to need a pull server. _True_, you won't always use a pull server in production. Sometimes, you'll doubtless have nodes that you simply push configurations to. That's fine - push mode is easy. But in what I expect will be many more cases, you'll want to use pull mode, and so that's where I'll start.
